...
import { formatCurrency, formatTurkishDate } from '../utils/financialCalculations';

interface AccountsViewProps {
  accounts: BankAccount[];
  incomes: Income[];
  onAddAccount: (acc: Omit<BankAccount, 'id' | 'createdAt' | 'updatedAt'>) => void;
  onUpdateAccount: (id: string, acc: Partial<BankAccount>) => void;
  onDeleteAccount: (id: string) => void;
  onAddIncome: (inc: Omit<Income, 'id' | 'createdAt'>) => void;
  onUpdateIncome: (id: string, inc: Partial<Income>) => void;
...
export const AccountsView: React.FC<AccountsViewProps> = ({
  ...
  const [accountType, setAccountType] = useState<BankAccount['accountType']>('vadesiz');
  ...
  const [incomeCategory, setIncomeCategory] = useState<IncomeCategory>('maas');
    ...
    <div className="space-y-8 pb-20 md:pb-10 max-w-7xl mx-auto">
      ...
      <div className="space-y-4">
        ...
        {accounts.length === 0 ? (
          <div className="p-10 bg-white rounded-2xl border border-dashed border-slate-200 text-center">
            ...
            <p className="text-sm font-bold text-slate-700">Henüz banka hesabı eklemediniz.</p>
            <p className="text-xs text-slate-500 mt-1">
                ...
                <div>
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{acc.accountName}</p>
                    </div>
      ...
      <div className="space-y-4 pt-4 border-t border-slate-200">
        ...
        {incomes.length === 0 ? (
          <div className="p-10 bg-white rounded-2xl border border-dashed border-slate-200 text-center">
            ...
            <p className="text-sm font-bold text-slate-700">Henüz gelir tanımlanmadı.</p>
            <p className="text-xs text-slate-500 mt-1">
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h4 className="font-bold text-sm text-slate-900 truncate">{inc.name}</h4>
                    <p className="text-xs text-slate-500 flex items-center gap-1.5 mt-0.5">
                      <span className="font-medium text-slate-700">
                        {inc.isRecurring ? 'Düzenli Gelir' : 'Tek Seferlik'}
                      </span>
                      <span>•</span>
                      <span>Her ayın {inc.dayOfMonth || 1}'i</span>
                    </p>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <h3 className="text-sm font-bold text-slate-900">Yeni Banka Hesabı Ekle</h3>
              <button
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    ...
                    <option value="birikim">Birikim / Fon</option>
                    <option value="vadeli">Vadeli Mevduat</option>
                    <option value="doviz">Döviz</option>
                    <option value="altin">Altın Hesabı</option>
                  </select>
                </div>
      ...
      {openIncomeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <h3 className="text-sm font-bold text-slate-900">Yeni Gelir Tanımla</h3>
              <button
            ...
            <form onSubmit={handleSaveIncome} className="p-6 space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="freelance">Freelance / Proje</option>
                  <option value="kira">Kira Geliri</option>
                  <option value="prim">Prim / İkramiye</option>
                  <option value="yatirim">Yatırım Getirisi</option>
