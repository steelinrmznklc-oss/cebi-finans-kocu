  ...
  const handleVoiceInput = async () => {
    ...
    try {
      ...
      if (permission.speechRecognition !== 'granted') {
        alert('Sesli kullanım için mikrofon izni gerekiyor.');
        return;
      }
      ...
      alert('Sesli giriş başlatılamadı.');
    } finally {
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-slate-800">
            ...
            <div className="p-2 bg-white rounded-lg border border-slate-200">
              ...
              >
                {snapshot.hasCashShortfall
                  ? '0 ₺ (Nakit Açığı)'
                  : formatCurrency(snapshot.dailySafeSpending)}
              </span>
            </div>
        ...
        {isLoading && (
          <div className="flex gap-3 max-w-md">
            ...
            <div className="p-3.5 rounded-2xl rounded-tl-xs bg-white border border-slate-200 text-xs text-slate-600 flex items-center gap-2 shadow-2xs">
              ...
              <span>
                CEBİ düşünüyor ve hesaplamalarını analiz ediyor...
              </span>
            </div>
          </div>
        )}
      ...
      <div className="bg-white p-4 rounded-b-2xl border-x border-b border-slate-200/90 shadow-xs shrink-0 space-y-2">
        <form onSubmit={handleSend} className="flex gap-2">
          <input
            id="coach-user-input"
            type="text"
            placeholder="Finansal bir soru sor veya 🎤 ile konuş..."
            value={inputText}
            disabled={isLoading}
            onChange={(e) => setInputText(e.target.value)}
            className="flex-1 min-w-0 px-4 py-3 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition outline-hidden disabled:opacity-50"
          />

          {/* VOICE BUTTON */}
          <button
            id="coach-voice-input-btn"
            type="button"
            onClick={handleVoiceInput}
            disabled={isLoading || isListening}
            className={`p-3 rounded-xl text-white font-bold transition flex items-center justify-center shrink-0 ${
              isListening
                ? 'bg-rose-500 animate-pulse'
                : 'bg-slate-800 hover:bg-slate-900'
            } disabled:opacity-40 disabled:cursor-not-allowed`}
            title={isListening ? 'Dinleniyor...' : 'Sesli mesaj'}
          >
            {isListening ? (
          ...
          <button
            ...
            className="px-4 sm:px-5 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm transition flex items-center gap-1.5 shadow-xs cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
          >
